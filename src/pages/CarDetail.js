...
const CarDetail = () => {
  ...
  return (
    <>
      ...
      {error && <div className="text-center">Sorry, some data are missing...</div>}
        ...
        <div className="p-6 w-11/12 mx-auto shadow-md rounded-lg text-sm font-light sm:w-full sm:max-w-[405px] sm:mx-0">
          <div className="my-10">
            <div className="max-w-72 max-h-52 flex items-center justify-center">
              <img src="/picar.png" alt="Car" className="object-contain max-w-full" />
            </div>
          </div>

          <h3 className="mb-2 text-sm font-bold">{carData.name ? carData.name : "Nama mobil"}</h3>

          <div className="text-sm font-light mb-6 flex items-center">
            <div className="flex items-center mr-4">
              <BsPeople style={{ display: "inline-block" }} size={18} /> <span className="ml-2">4 orang</span>
            </div>
            <div className="flex items-center mr-4">
              <BsGear style={{ display: "inline-block" }} size={18} /> <span className="ml-2">Manual</span>
            </div>
            <div className="flex items-center">
              <BsCalendarDate style={{ display: "inline-block" }} size={18} /> <span className="ml-2">Tahun 2020</span>
            </div>
          </div>

          <div className="flex justify-between mb-6">
            <p>Total</p>
            <p className="font-bold">Rp. {carData.price ? carData.price?.toLocaleString("id-ID") : "430.000"}</p>
          </div>

          <button className="w-full px-3 py-2 bg-[#5CB85F] rounded-sm text-white font-bold ">Lanjutkan Pembayaran</button>
        </div>
      </div>
    </>
  );
